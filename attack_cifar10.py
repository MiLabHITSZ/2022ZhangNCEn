...
from attack_engine import prepare_attack_example_loss_ensemble, prepare_attack_example_output_ensemble, \
    prepare_attack_example_single
...
path_suffix_list = ['resnet20']
loss_type_list = ['norm_cos', 'norm_cos1', 'norm_cos2', 'norm_cos3', 'norm_cos4', 'norm_cos5',
...
def load_model_list(args):
    ...
    if args.train_type == 'single':
        if args.loss_type == 'ce':
            ...
            for i, name in enumerate(model_name):
                logging.info(os.path.join(args.save_model_path,
                                          model_name[i] + '_' + args.loss_type + '_single_checkpoint.pth.tar'))
                dict = torch.load(os.path.join(args.save_model_path,
                                               model_name[i] + '_' + args.loss_type + '_single_model_best.pth.tar'))
                model_list[i].load_state_dict(dict)
        return model_list
        ...
        for i, name in enumerate(model_name):
            if args.para_flag == True:
                ...
                dict = torch.load(os.path.join(args.save_model_path, str(args.para_config) + '_' + model_name[
                    i] + '_' + args.dynamic_type + '_' + args.loss_type + '_loss_ensemble_' + args.model_type + '.pth.tar'))
            else:
                dict = torch.load(os.path.join(args.save_model_path, model_name[
                    i] + '_' + args.dynamic_type + '_' + args.loss_type + '_loss_ensemble_' + args.model_type + '.pth.tar'))
...
def main_loss_ensemble(data_loader_test):
    ...
    if args.para_flag == True:
        inputs = np.load(os.path.join(args.prepare_attack_path,
                                      'loss_ensemble',
                                      str(args.para_config) + '_' + args.dynamic_type + '_' + args.loss_type + '_' + str(
                                          model_name) + '_' + 'x.npy'))
        ...
        inputs = np.load(os.path.join(args.prepare_attack_path,  # ../prepare_attack_data/fashion_mnist/resnetmix
                                      'loss_ensemble',
                                      args.dynamic_type + '_' + args.loss_type + '_' + str(model_name) + '_' + 'x.npy'))
        targets = np.load(os.path.join(args.prepare_attack_path,
                                       'loss_ensemble',
                                       args.dynamic_type + '_' + args.loss_type + '_' + str(
                                           model_name) + '_' + 'y.npy'))
...
if __name__ == '__main__':
    ...
    for path_suffix in path_suffix_list:
        ...
        args.save_model_path = '../checkpoint/cifar10'
        args.save_adv_path = '../save_adv/cifar10'
        args.prepare_attack_path = '../prepare_attack_data/cifar10'

        args.para_config = {'alpha': args.alpha}
        log_save_path = os.path.join(args.save_model_path, args.path_suffix + '_log',
                                     '')  # ../checkpoint/fashion_mnist/resnetmix_log
        ...
        # else:
        #     init_log(os.path.join(log_save_path,                ##../checkpoint/fashion_mnist/resnetmix_log/normal_norm_cos1loss_ensemble.log
        ...
        for loss_type in loss_type_list:
            ...
            if args.para_flag == True:
                ...
                init_log(os.path.join(log_save_path,
                                      ##../checkpoint/fashion_mnist/resnetmix_log/normal_norm_cos1loss_ensemble.log
                                      args.dynamic_type + '_' + args.loss_type + args.train_type + '.log'))
